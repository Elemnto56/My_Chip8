class Chip8 {
    ...
    V = new Uint8Array(16).fill(0); //registers
    I = 0;
    stack = new Uint16Array(16);
    SP = -1; // stack pointer
    DT = 0; // delay timer
    ST = 0; // sound timer
    disp = new Array(64 * 32).fill(0);
    key_map = new Map([
        ...
        switch (opcode & 0xF000) {
            ...
            case 0xf000:
                switch (opcode & 0x00FF) {
                    ...
                    case 0x0a:
                        {
                            let prev = this.key_map;
                            let found = false;
                            while (true) {
                                this.key_map.forEach((val, key) => {
                                    if (prev[key] != this.key_map[key]) {
                                        this.V[x] = key;
                                        found = true;
                                    }
                                });
                    ...
                    case 0x55:
                        for (let i = 0; i < x; i++) {
                            this.memory.set([this.V[i]], this.I);
                            this.I++;
                        }
...
let fInput = document.getElementById("rom_file");
